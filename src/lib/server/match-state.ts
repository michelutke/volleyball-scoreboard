import type { MatchState, SetScore } from '$lib/types.js';
import type { matches, scores } from '$lib/server/db/schema.js';

type MatchRow = typeof matches.$inferSelect;
type ScoreRow = typeof scores.$inferSelect;

export function toResetMatchState(match: MatchRow): MatchState {
	return toMatchState(match, {
		homePoints: 0,
		guestPoints: 0,
		homeSets: 0,
		guestSets: 0,
		currentSet: 1,
		setScores: [],
		serviceTeam: 'home'
	} as ScoreRow);
}

export function toMatchState(match: MatchRow, score: ScoreRow): MatchState {
	return {
		matchId: match.id,
		homeTeamName: match.homeTeamName,
		guestTeamName: match.guestTeamName,
		homeJerseyColor: match.homeJerseyColor,
		guestJerseyColor: match.guestJerseyColor,
		showJerseyColors: match.showJerseyColors,
		overlayBg: match.overlayBg,
		overlayBg2: match.overlayBg2,
		overlayBgGradient: match.overlayBgGradient,
		overlayText: match.overlayText,
		overlayRounded: match.overlayRounded,
		overlayDivider: match.overlayDivider,
		overlaySatsBg: match.overlaySatsBg,
		overlaySetScoreBg: match.overlaySetScoreBg,
		homeTeamLogo: match.homeTeamLogo ?? null,
		guestTeamLogo: match.guestTeamLogo ?? null,
		scoreColor: match.scoreColor,
		scoreColor2: match.scoreColor2,
		scoreColorGradient: match.scoreColorGradient,
		designTemplateId: match.designTemplateId ?? null,
		homePoints: score.homePoints,
		guestPoints: score.guestPoints,
		homeSets: score.homeSets,
		guestSets: score.guestSets,
		currentSet: score.currentSet,
		setScores: score.setScores as SetScore[],
		serviceTeam: score.serviceTeam,
		showSetScores: match.showSetScores,
		status: match.status
	};
}
